...
public:
	...
	static void AutoLog(int logLevel, const std::string& msg) { // автологгер
		if (Log::flag[logLevel] == true) { // галочка на INFO стоит
		...
		if (flag[logLevel] == true) { // галочка на DEBUG стоит
			Log::SetLogLevel(static_cast<Log::Level>(logLevel)); // открываем уровень
		...
		if (flag[logLevel] == true) { // галочка на RELEASE стоит
			Log::SetLogLevel(static_cast<Log::Level>(logLevel)); // открываем уровень
	...
	static void AutoLog(int logLevel, const std::string& msg, const std::string& enmbr) { // ERROR
		if (flag[logLevel] == true) { // галочка на ERROR стоит
			Log::SetLogLevel(static_cast<Log::Level>(logLevel)); // открываем уровень
